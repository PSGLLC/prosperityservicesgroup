...
//
// Mirrors this repo's functions/api/guide-lead.ts, which in turn mirrors
// PSGLLC/learnmedicare's functions/api/guide-lead.ts. Static Astro build,
// Cloudflare Pages Functions provide the one dynamic endpoint. GHL webhook
// URL stays server-side (env secret), never exposed to the browser.

interface Env {
  GHL_PROSPERITY_CONTACT_WEBHOOK_URL: string;
}
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  const payload = {
    ...
    tags: ["prosperity-inquiry"],
    source: "prosperityservicesgroup-contact-page",
    pagePath: String(body.pagePath ?? "/contact"),
    submittedAt,
  };

  if (!env.GHL_PROSPERITY_CONTACT_WEBHOOK_URL) {
    return json({ ok: false, error: "Message delivery is not configured yet." }, 500);
  }

  const delivered = await forwardToGHL(env.GHL_PROSPERITY_CONTACT_WEBHOOK_URL, payload);
  if (!delivered) {
